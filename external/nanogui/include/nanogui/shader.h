...
protected:
	/// Definition of buffer properties (without the actual data)
	...
	std::unordered_map<std::string, BufferDefinition> m_buffer_definitions;
	
	/// Persisted buffers that remain across draw calls
	std::unordered_map<std::string, std::unordered_map<int, Buffer>> m_persisted_buffers;
	
	/// Queued buffers that are cleared after each draw call
