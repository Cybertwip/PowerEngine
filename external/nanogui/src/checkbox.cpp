...
Vector2i CheckBox::preferred_size(NVGcontext *ctx) {
...
void CheckBox::draw(NVGcontext *ctx) {
    ...
    nvgFontFace(ctx, "sans");
    nvgFillColor(ctx,
                 m_enabled ? theme().m_text_color : theme().m_disabled_text_color);
    ...
    if (m_checked) {
        ...
        nvgFillColor(ctx, m_enabled ? theme().m_icon_color
                                   : theme().m_disabled_text_color);
        nvgTextAlign(ctx, NVG_ALIGN_CENTER | NVG_ALIGN_MIDDLE);
        nvgText(ctx, m_pos.x() + m_size.y() * 0.5f + 1,
                m_pos.y() + m_size.y() * 0.5f, utf8(theme().m_check_box_icon).data(),
                nullptr);
    }
}
