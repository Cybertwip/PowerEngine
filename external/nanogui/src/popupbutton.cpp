...
void PopupButton::draw(NVGcontext* ctx) {
    ...
    if (m_chevron_icon) {
        ...
        nvgFillColor(ctx, m_enabled ? text_color : NVGcolor(theme().m_disabled_text_color));
...
void PopupButton::perform_layout(NVGcontext *ctx) {
    ...
    if (parent_window) {
        ...
        if (m_popup->side() == Popup::Right)
			m_popup->set_anchor_pos(Vector2i(anchor_size + m_size.x(), pos_y));
        else
...
void PopupButton::set_side(Popup::Side side) {
    if (m_popup->side() == Popup::Right &&
        m_chevron_icon == theme().m_popup_chevron_right_icon)
        set_chevron_icon(theme().m_popup_chevron_left_icon);
    else if (m_popup->side() == Popup::Left &&
             m_chevron_icon == theme().m_popup_chevron_left_icon)
        set_chevron_icon(theme().m_popup_chevron_right_icon);
