...
m_framebuffer_handle(0) {
	...
	if (has_screen && !has_texture) {
		...
		if (status != GL_FRAMEBUFFER_COMPLETE) {
			...
			std::cerr << "RenderPass::RenderPass(): framebuffer is marked as incomplete: "
			<< std::string(reason) << std::endl;
			
			// Log which attachments are incomplete
			for (size_t i = 0; i < m_targets.size(); ++i) {
				if (!m_targets[i]) {
					std::cerr << "Incomplete attachment at index: " << i << std::endl;
				}
			}
...
void RenderPass::begin() {
	...
	std::vector<int> pixels(16);
	
	int height = m_framebuffer_size[1];
	int x = 120;
	int y = 240;
	// Adjust the y-coordinate according to OpenGL's coordinate system
	int adjusted_y = height - y - 2;
	
	// Set the read buffer to the appropriate color attachment
	//CHK(glReadBuffer(GL_COLOR_ATTACHMENT0 + 1));
	
	// Read the pixel data from the specified region
	//CHK(glReadPixels(x - 2, adjusted_y, 4, 4, GL_RED_INTEGER, GL_INT, pixels.data()));
