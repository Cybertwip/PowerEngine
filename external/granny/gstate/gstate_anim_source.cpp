...
anim_source::BindToCharacter(gstate_character_instance* Instance)
{
    ...
    if (TrackGroupIndex != -1)
    {
        ...
        // Separate out the loops for these, just to keep it simple...  Next scalar tracks
        {for (int EdgeIdx = 1; EdgeIdx < GetNumOutputs(); ++EdgeIdx)
        {
            int const TrackSlotIdx = EdgeIdx - 1;
                       ...
                       char const*    EdgeName)
{
    ...
    GStateAssert(GetNumOutputs() == 0 ||
                 (EdgeType == EventEdge || EdgeType == ScalarEdge));
    ...
    else
    {
        ...
        // EventTracks
        {
            granny_int32 TrackCount = PreviousCount;
            granny_text_track*& NewRef = QVecPushNewElement(TrackCount, m_EventTracks);
            NewRef = 0;
        }
...
anim_source::DeleteOutput(granny_int32x OutputIndex)
{
    ...
    // Trickery allows code sharing...
    { // events
        granny_int32 Count = PreviousCount;
        QVecRemoveElement(ArrayIdx, Count, m_EventTracks);
    }
    { // scalars
        granny_int32 Count = PreviousCount;
        QVecRemoveElement(ArrayIdx, Count, m_ScalarTracks);
    }
...
bool GSTATE
anim_source::SetEventTrackName(granny_int32x ForOutput,
                               char const* TrackName)
{
    ...
    // Common work
    {
        ...
        GStateDeallocate(m_anim_sourceToken->EventEdges[ArrayIndex].TrackName);
        m_anim_sourceToken->EventEdges[ArrayIndex].TrackName = 0;

        if (TrackName != 0)
        {
            GStateCloneString(m_anim_sourceToken->EventEdges[ArrayIndex].TrackName, TrackName);
        }
    }
