...
state_machine::AddOutputToChildren(int NewOutputIdx)
{
    ...
    else
    {
        ...
        GStateAssert(EdgeType == ScalarEdge || EdgeType == EventEdge);
...
void GSTATE
state_machine::AdvanceT(granny_real32 CurrentTime, granny_real32 DeltaT)
{
    ...
    if (Node)
    {
        ...
        {for (int Pass = 0; Pass < 2 && Activated == 0; ++Pass)
        {
            {for (int Idx = 0; Idx < Node->GetNumTransitions(); ++Idx)
            {
                ...
                if (Possible->ShouldActivate(Pass, Trigger_Automatic, CurrentTime, DeltaT))
                {
                    //int External = Node->GetNthExternalOutput(0);
                    m_Active = Activated = Possible;
                    Possible->Activate(CurrentTime);
                    break;
                }
            }}
        }}
    ...
    else
    {
        ...
        if (m_state_machineToken->StartState != eInvalidChild)
        {
            ...
            if (State)
            {
                ...
                State->Activate(External, CurrentTime);
            }
        }
    }
...
state_machine::AddOutput(node_edge_type EdgeType, char const* EdgeName)
{
    ...
    if (GetNumOutputs() != 0 && (EdgeType != ScalarEdge && EdgeType != EventEdge))
    {
