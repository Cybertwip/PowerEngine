...
#if defined(__RADX86__) || defined(__RADX64__) || defined(__RADPSP__) || defined(__RADNDS__) || defined(__RADSPU__)
...
RADDEFFUNC void VarBitsCopy(VARBITS* dest,VARBITS* src,U32 size);
    ...
    static RADINLINE U32 getbitlevel128( U32 n )
    {
      count_leading_zeros( n, n );
      return( 32 - n );
    }
  
    static RADINLINE U32 getbitlevel( U32 n )
    {
      n = n & 0xffff;
      count_leading_zeros( n, n );
      return( 32 - n );
...
{                                                               \
  if (((vb).bitlen)<(len)) {                                    \
    BITSTYPE nb=*((BITSTYPE* RADRESTRICT)((vb).cur));  \
    ((vb).bits)=nb>>((len)-((vb).bitlen));                      \
    ((vb).bitlen)=((vb).bitlen)+BITSTYPELEN-(len);              \
    ((vb).cur)=((char*)((vb).cur))+BITSTYPEBYTES;               \
  } else {                                                      \
