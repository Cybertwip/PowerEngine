...
struct ansi_file_writer : public file_writer
{
    FILE *ANSIFileHandle;
};
               ...
               file_writer_seek_type SeekType)
{
    ...
    ansi_file_writer &AnsiWriter = (ansi_file_writer&)Writer;
    Assert(!AnsiWriter.CRCing || (SeekType == SeekCurrent && OffsetInUInt8s == 0));
          ...
          file_writer &Writer, int32x UInt8Count, void const *WritePointer)
{
    ...
    ansi_file_writer &AnsiWriter = (ansi_file_writer&)Writer;
    if(AnsiWriter.CRCing)
    {
        // We're CRCing, so we need to update our current CRC value
        AddToCRC32(AnsiWriter.CRC, UInt8Count, WritePointer);
    }
                   ...
                   file_writer &Writer)
{
    ...
    AnsiWriter.CRCing = true;
    BeginCRC32(AnsiWriter.CRC);
}
                 ...
                 file_writer &Writer)
{
    ...
    AnsiWriter.CRCing = false;
    EndCRC32(AnsiWriter.CRC);

    return(AnsiWriter.CRC);
}
