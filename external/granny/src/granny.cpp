...
GRANNY_DYNLINK(char *) GrannyMemoryArenaPushString(granny_memory_arena *Arena, char const *String)
{
#ifdef GRANNY_THREAD_CHECKS
    if (!GrannyThreadAllowedToCallGranny())
    {
    ...
    // Use Granny's string length function
    granny_int32x Length = (granny_int32x)StringLength(String) + 1;

    // Allocate memory using GrannyMemoryArenaPush
    char *AllocatedMemory = (char *)GrannyMemoryArenaPush(Arena, Length);

    if (AllocatedMemory)
    {
        // Use Granny's memory copy function with correct argument order
        Copy(Length, String, AllocatedMemory);
    }
