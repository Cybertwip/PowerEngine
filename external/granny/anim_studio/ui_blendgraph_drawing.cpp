...
DoBlendGraphNode(node* Node, bool Selected, LuaRect& DrawnRect)
{
    ...
    ControlIDGen IDGen("bg_slider_controls_%x_%d", edit::TokenizedToID(Node), Node->GetNumOutputs());
    ...
    if (UIAreaPush(ParentRect, ChildRect))
    {
        ...
        {for (int EdgeIdx = 0; EdgeIdx < NumOutputs; ++EdgeIdx)
        {
            ...
            if (Param)
            {
                ...
                float Min, Max;
                float Value = Param->SampleScalarOutput(EdgeIdx, 0);
                Param->GetMinMax(EdgeIdx, &Min, &Max);

                LuaRect Rect(5, CurrY, 100, SmallSliderHeight());
                if (SmallSliderOn(Min, Max, Value, Rect, IDGen))
                {
                    PushUndoPos("change parameter", eSliderMotion, Param->GetUID());
                    Param->SetParameter(EdgeIdx, Value);
                }
            }
...
{
    int32x EdgeTypes[EdgeTypeCount] = { PoseEdge, ScalarEdge, MaskEdge, EventEdge };
