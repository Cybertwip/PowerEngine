...
edit::Save(bool QueryAlways)
{
    ...
    if (!Writer)
    {
        FadePrintf(gErrorColor, 3.0f, "Failed to open '%s' for writing", SavePath.c_str());
    ...
    {
        FadePrintf(LuaColor(1.f, 0.2f, 0.2f), 3.0f, "Failed to save to: '%s'", SavePath.c_str());
...
// =================================================================================
// static int
// Edit_SetRootMachineName(lua_State* L)
// {
//     int Index            = lua_tointeger(L, -2) - 1;  // account for +1 addressing
//     char const* NewName  = lua_tostring(L, -1);

//     if (Index < 0 || Index >= (int)RootMachines.size())
//     {
//         return LuaBoolean(L, false);
//     }

//     Assert(RootMachines[Index]);
//     RootMachines[Index]->SetName(NewName);
...
// static int
// Edit_GetRootMachineNames(lua_State* L)
// {
//     vector<char const*> Names;
//     int CurrentMachine = -1;

//     {for (size_t Idx = 0; Idx < RootMachines.size(); ++Idx)
//     {
//         Names.push_back(RootMachines[Idx]->GetName());
//         if (CurrentRootMachine == RootMachines[Idx])
//             CurrentMachine = Idx;
//     }}
//     Assert(CurrentMachine != -1 || Names.size() == 0);

//     if (CurrentRootMachine != 0)
//     {
//         Assert(CharacterInstance == 0 || GStateGetActiveStateMachine(CharacterInstance) != 0);
//         Assert(CharacterInstance == 0 ||
//                StringsAreEqual(GStateGetActiveStateMachine(CharacterInstance)->GetName(),
//                                CurrentRootMachine->GetName()));
//     }

//     PushStringArray(L, Names);
//     lua_pushinteger(L, CurrentMachine+1);  // -> lua array index

//     return 2;
// }

// static int
// Edit_SetRootMachine(lua_State* L)
// {
//     PushUndoPos("Change root machine");

//     int32x NewRootIdx = (int32x)lua_tointeger(L, -1) - 1;  // <- lua array index
//     if (NewRootIdx < 0 || NewRootIdx >= int(RootMachines.size()))
//         return LuaBoolean(L, false);

//     state_machine* NewMachine = RootMachines[NewRootIdx];
//     if (NewMachine != CurrentRootMachine)
//         SetRootMachine(RootMachines[NewRootIdx]);

//     return LuaBoolean(L, true);
// }

// static int
// Edit_CreateNewMachine(lua_State* L)
// {
//     PushUndoPos("Add root machine");
//     lua_pushboolean(L, (NewRootMachine(0) != 0));
//     return 1;
// }
...
Edit_ReplaceNodeWith(lua_State* L)
{
    ...
    node* NewNode = AddNodeImpl(L, NodeType, Node->GetName(), CreatePoint);
    if (NewNode == 0)
    {
        PopUndoPos();
        return LuaNode(L, 0);
...
Edit_PushUndoPos(lua_State* L)
{
    ...
    PushUndoPos(PositionName, EUndoClass(PositionType));
...
Edit_SetCurrentAnimationSet(lua_State* L)
{
    ...
    PushUndoPos("SetCurrentAnimationSet");
...
edit::LookForChangedSources()
{
                               ...
                               ChangedFileInfo))
    {
        if (ChangedRef->ExpectedAnimCount != 0 && ChangedRef->AnimCRC != 0)
        {
            ...
            {
                FadePrintf(gErrorColor, 5.0f, "'%s': incompatible change IGNORED", ChangedRef->SourceFilename);
...
Edit_SetParameterOutputProperties(lua_State* L)
{
    ...
    Parameters->SetMinMax(Output, Min, Max);
...
Edit_SetCommentPosition(lua_State* L)
{
    ...
    PushUndoPos("comment move", eCommentMove);
    MarkModified();
