...
Edit_TransitionEdit(lua_State* LuaState)
{
    ...
    // Duration
    {
        ...
        LuaRect ControlRect(ControlX, CurrY, 200, 26);
        if (SliderOn(0, 2, Duration, ControlRect, IDGen))
        {
            PushUndoPos("Transition duration", eSliderMotion, Transition->GetUID());
            edit::MarkModified();
    ...
    if (Transition->GetTransitionType() == Transition_OnLoop)
    {
        ...
        LuaRect ControlRect(ControlX, CurrY, 200, 26);
        if (SliderOn(0, 1, Probability, ControlRect, IDGen))
        {
            PushUndoPos("Transition probability", eSliderMotion, Transition->GetUID());
            edit::MarkModified();
