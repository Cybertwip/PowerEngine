...
CreateNodeDragType  = 80085
CreateStateDragType = 8008135


function GetPersistentRatio(Name, DefaultNum, DefaultDenom)
   local  N = App_GetPersitentInt(Name .. "_Num",   DefaultNum)
   local  D = App_GetPersitentInt(Name .. "_Denom", DefaultDenom)

   return (N/D)
end

function SetPersistentRatio(Name, Num, Denom)
   App_SetPersitentInt(Name .. "_Num",   Num)
   App_SetPersitentInt(Name .. "_Denom", Denom)
end
...
function GraphRegion(ID)
   return NodeDisplay(ID)
end
