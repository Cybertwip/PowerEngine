...
ComputeStateNodeRect(node* Node)
{
    ...
    {
        // Sliders
        Height += Params->GetNumOutputs() * 16;
    }
    ...
    {
        // Buttons
        Height += Source->GetNumOutputs() * 24;
    }
...
DoStateNode(node* Node, bool Selected, bool IsStart, bool IsCurrent, LuaRect& DrawnRect)
{
    ...
    bool NodeIsValidState = IsValidState(Node);
    int  DragFringe       = NodeIsValidState ? 10 : 0;

    // Only used for parameters...
    ControlIDGen IDGen("node_slider_controls_%x_%d", edit::TokenizedToID(Node), Node->GetNumOutputs());
    ...
    if (UIAreaPush(ParentRect, ChildRect))
    {
        ...
        // Lua interactions...
        {
            ...
            RegisterNodeMove(L, Node, ChildRect, IconRect, DragFringe, IsValidState(Node));
        }
        ...
        if (Params)
        {
            ...
            {
                float Min, Max;
                Params->GetMinMax(Idx, &Min, &Max);

                float Value = Params->SampleScalarOutput(Idx, 0);

                LuaRect Rect(10, IconRect.y + IconRect.h + Idx * Height, ChildRect.w - 20, Height);
                if (SmallSliderOn(Min, Max, Value, Rect, IDGen))
                {
                    PushUndoPos("change parameter", eSliderMotion, Params->GetUID());
...
DoTransitions(node* Node)
{
    ...
    {for (int32x TransitionIdx = 0; TransitionIdx < Node->GetNumTransitions(); ++TransitionIdx)
    {
        ...
        DrawConnection(start, end, edit::IsSelected(Transition));
    }}
...
DrawTransitionDrag(lua_State* L)
{
    ...
    if (ActiveDrag)
    {
        ...
        DrawConnection(start, end, false);
    }
}
