...
local function RenderSourceFile(ID, Entry, Index)
   ...
   if (not Entry.Loaded) then
      TipString =
         (TipString .. "\n" ..
          ...
          "  You can manually replace the source\n" ..
          "  animation by right clicking the name.")
   ...
   if (not Interactions.Active()) then
      ...
      if (Entry.Loaded) then
         ...
         Item.ReplaceAction =
            Interactions.RegisterDefClipped(Item.ReplaceAction or
                                            MakeReplaceSource(Item, UIRect, Index))
      end
   end
...
local function SlotEntries(ID, MidPoint)
   ...
   for i,v in ipairs(AnimationSlotList) do
      ...
      ToolTip.Register("Drag animations to change", NameRect)
      ...
      if (v.SourceIdx) then
         if (not v.SourceString) then
            if (v.ExpectedName) then
               ...
               if (SourceList[v.SourceIdx].ValidAnims[v.AnimIdx] ~= nil) then
                  v.SourceString = (SourceList[v.SourceIdx].ValidAnims[v.AnimIdx].ShortName .. " in " .. SourceList[v.SourceIdx].Root)
                  v.Mismatch = (v.ExpectedName ~= SourceList[v.SourceIdx].ValidAnims[v.AnimIdx].FullName)
               else
                  v.SourceString = (v.ExpectedName .. " in " .. SourceList[v.SourceIdx].Root)
                  v.Mismatch = true
               end
            else
               v.SourceString = (SourceList[v.SourceIdx].ValidAnims[v.AnimIdx].ShortName ..
