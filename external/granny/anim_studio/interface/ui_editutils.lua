...
local function HandleKey(Item, Event)
   ...
   elseif (Event.Ctrl and Event.Key == KeyForChar("v")) then
      ...
      STB_EditKey(Item.STBEdit, Event.Key, Event.Shift, Event.Ctrl)
   end
end
...
function EditEventLoop(Item, Rect, StartDownAction)
   local Success = true
   while (true) do
...
function MakeEdit(args)
   ...
   local function EditCoroutine(Event)
      ...
      local Success = EditEventLoop(Item, Rect, StartDownAction)
