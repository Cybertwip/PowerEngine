...
local ScrollWidth = 18

local function ValidScrollArea(AreaRect)
   return ((AreaRect.w >= 2 * ScrollWidth) and (AreaRect.h >= 2 * ScrollWidth))
end
...
local function Do(ID, Zoomable, Offset, ChildDraw)
   ...
   if (ValidScrollArea(BaseArea) == false) then
      print("ScrollArea width/height must be at least " .. 2*ScrollWidth .. " pixels")
      return 0
   end
   ...
   if (not Item.ScrollActive) then
      ...
      if (not FullyContains(Item.ChildArea.x, Item.ChildArea.w,
                            Item.RenderedArea.x, Item.RenderedArea.w)) then
         Item.HorizActive = true
         BaseArea.h  = BaseArea.h - ScrollWidth
         Item.RenderedArea.h = Item.RenderedArea.h - ScrollWidth
      end

      if (not FullyContains(Item.ChildArea.y, Item.ChildArea.h,
                            Item.RenderedArea.y, Item.RenderedArea.h)) then
         Item.VertActive = true
         BaseArea.w  = BaseArea.w - ScrollWidth
         Item.RenderedArea.w = Item.RenderedArea.w - ScrollWidth

         -- recheck this, just in case, since we've changed the variables...
         if (not Item.HorizActive and
   ...
   if (Item.VertActive) then
      ...
      Item.Offset.y, VertScrolling =
         Scrollbar.DoVertical(IDItem.CreateSub(ID,"vertbar"),
                              MakeRect(BaseArea.w, 0, ScrollWidth, BaseArea.h),
                              Item.Offset.y,
   ...
   if (Item.HorizActive) then
      -- Draw vertical scroller
      Item.Offset.x =
         Scrollbar.DoHorizontal(IDItem.CreateSub(ID,"horizbar"),
                                MakeRect(0, BaseArea.h, BaseArea.w, ScrollWidth),
                                Item.Offset.x,
   ...
   DrawRect(MakeRect(UIAreaGet().w - ScrollWidth,
                     UIAreaGet().h - ScrollWidth,
...
ScrollArea = {
   Do = function(ID, Offset, ChildDraw)
           return Do(ID, false, Offset, ChildDraw)
        end,
   
   DoZoomable = function(ID, Offset, ChildDraw)
                   return Do(ID, true, Offset, ChildDraw)
                end
}
