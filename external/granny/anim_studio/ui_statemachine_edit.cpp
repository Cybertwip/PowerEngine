                 ...
                 node_edge_type       Type)
{
    ...
    if (IncludeParameters)
    {
        ...
        {
            parameters* AsParameters = GSTATE_DYNCAST(Machine->GetChild(Idx), parameters);

            if (AsParameters == 0)
                continue;

            {for (int Idx = 0; Idx < AsParameters->GetNumOutputs(); ++Idx)
            {
                if (AsParameters->GetOutputType(Idx) == Type)
                {
                    // Setup the name as "edge name (on 'nodename')" for the parameters.
                    // That's 8 extra characters for those playing around at home
                    int const TotalLen = strlen(AsParameters->GetName()) + 8 + strlen(AsParameters->GetOutputName(Idx)) + 1;

                    char* LabeledName = (char*)GStateAlloc(TotalLen);
                    sprintf(LabeledName, "%s (on '%s')", 
                            AsParameters->GetOutputName(Idx),
                            AsParameters->GetName());

                    EdgeNames.push_back(LabeledName);
                    EdgeNodes.push_back(AsParameters);
                    EdgeIndices.push_back(Idx);
                }
            }}
        }}
                  ...
                  ControlIDGen const& ParentIDGen)
{
    ...
    // strings after this is complete.
    {
        ...
        vector<node*>       EdgeNodes;
        vector<int>         EdgeIndices;
        GatherValidEdges(ScalarCompare->GetOwner(), true, EdgeNames, EdgeNodes, EdgeIndices, ScalarEdge);
                   ...
                   ControlIDGen const& ParentIDGen)
{
    ...
    // strings after this is complete.
    {
        ...
        vector<node*>       EdgeNodes;
        vector<int>         EdgeIndices;
        GatherValidEdges(EventTriggered->GetOwner(), false, EdgeNames, EdgeNodes, EdgeIndices, EventEdge);

        bool Changed = false;

        int32x      OutputIndex = -1;
        char const* EventString = 0;
        EventTriggered->GetTriggerEvent(&OutputIndex, &EventString);

        int32x CurrentEntry = -1;
        {for (int Idx = 0; Idx < SizeI(EdgeIndices); ++Idx)
        {
            if (EdgeIndices[Idx] == OutputIndex)
            {
                CurrentEntry = Idx;
                break;
            }
        ...
        {
            Changed = true;
            OutputIndex = EdgeIndices[CurrentEntry];
            EventString = 0;
        }
        AdvanceYLastControl(CurrY);
        ...
        if (OutputIndex != -1 &&
            EventTriggered->GetOwner()->GetAllEvents(OutputIndex, Buffer, 128, &TotalEvents))
        {
            CurrentEntry = -1;
            ...
            {
                Changed = true;
                EventString = EventNames[CurrentEntry];
            }
        }
        else
        {
        ...
        if (Changed)
        {
            ...
            edit::MarkModified();
            EventTriggered->SetTriggerEvent(OutputIndex, EventString);
        }
...
EditStateMachineNode(lua_State* L, state_machine* Machine, int StartY)
{
    ...
    if (GSTATE_DYNCAST(Machine->GetParent(), blend_graph))
    {
        ...
        node_edge_type EdgeTypes[] = { ScalarEdge, EventEdge };
        char const* EdgeNames[] = { "Scalar", "Event" };
    ...
    else
    {
        ...
        {
            int32x LabelX, ControlX;
            LabeledControlsAtWidth(max(LabelWidth("Scalar"), LabelWidth("Event")),
                                   &LabelX, &ControlX);
        ...
        //     [add new event publish]
        {
            ...
            int32x LeftX, RightX;
            LabeledControlsAtWidth(CenterX(drawArea), &LeftX, &RightX);

            char const* AddScalarLabel = "Add Scalar";
            char const* AddEventLabel  = "Add Event";

            int32x ASWidth = SmallButtonWidth(AddScalarLabel);
            if (SmallButtonAt(AddScalarLabel, LuaPoint(LeftX - ASWidth, CurrY), AllowedToEditParams, IDGen))
            {
            ...
            if (SmallButtonAt(AddEventLabel, LuaPoint(RightX, CurrY), AllowedToEditParams, IDGen))
            {
