               ...
               vector<animation_slot*>& AnimationSlots)
{
    ...
    // Speed
    {
        ...
        if (SliderOn(0, 10, Speed, LuaRect(ControlX, CurrY, 200, 26), IDGen))
        {
            PushUndoPos("Animation option", eSliderMotion, Animation->GetUID());
            edit::MarkModified();
...
SharedParametersEdit(lua_State* LuaState, parameters* Parameters)
{
    ...
    {for (int Idx = 0; Idx < Parameters->GetNumOutputs(); ++Idx)
    {
        ...
        Parameters->GetMinMax(Idx, &MinVal, &MaxVal);
        if (SliderOn(MinVal, MaxVal, Param, SliderRect, IDGen))
        {
            PushUndoPos("Change parameter", eSliderMotion, Parameters->GetUID());
            edit::MarkModified();
        ...
        // Min/Current/Max
        {
            // Min
            {
                ...
                if (EditLabelOn(MinStr, LuaRect(MinStart, CurrY, MinMaxValWidth, 24), IDGen))
                {
                    ...
                    if (MinVal > MaxVal)
                        MaxVal = MinVal;
                    Parameters->SetMinMax(Idx, MinVal, MaxVal);
...
TriggerEventPoke(lua_State* L)
{
    ...
    Source->PokeEventByIdx(EventIdx);
                   ...
                   bool         VisitorIsBlendGraph)
{
            ...
            if (Animation->GetOutputType(Idx) == EventEdge)
            {
                ...
                if (TrackGroup)
                {
                    ...
                    if (ITunesComboboxOn(EventTrackNames, CurrentEntry, Rect, IDGen))
                    {
                        ...
                        {
                            Animation->SetEventTrackName(Idx, EventTrackNames[CurrentEntry]);
                            if (VisitorIsBlendGraph)
                                Animation->SetOutputName(Idx, EventTrackNames[CurrentEntry]);
                        }
                        else
                        {
                            Animation->SetEventTrackName(Idx, NULL);
                            if (VisitorIsBlendGraph)
    ...
    if (VisitorIsBlendGraph)
    {
        ...
        // Abuse the label/control mechanism a bit here...
        int32x LeftX, RightX;
        LabeledControlsAtWidth(CenterX(drawArea), &LeftX, &RightX);

        char const* AddScalarLabel = "Add Scalar Output";
        int AddScalarWidth = SmallButtonWidth(AddScalarLabel);

        if (SmallButtonAt(AddScalarLabel, LuaPoint(LeftX - AddScalarWidth, CurrY), true, IDGen))
        {
        ...
        if (SmallButtonAt("Add Event Output", LuaPoint(RightX, CurrY), true, IDGen))
        {
...
blendnode_edit_visitor::VisitNode(blend_graph* BlendGraph)
{
    ...
    char const* EdgeNames[] = { "Pose", "Scalar", "Mask", "Event" };
...
blendnode_edit_visitor::VisitNode(blend* Blend)
{
    ...
    {
        LabelAt("Phase locked:", LuaPoint(LabelX, CheckboxLabelAdjust(CurrY)), eRight);
...
blendnode_edit_visitor::VisitNode(pose_storage* PoseStorage)
{
    ...
    real32 Time = 0;
    if (SliderOn(0, Duration, Time, SliderRect, IDGen))
    {
        if (InputConnected)
        {            
            PushUndoPos("Change Capture", eSliderMotion, PoseStorage->GetUID());
            edit::MarkModified();
...
blendnode_edit_visitor::VisitNode(constant_velocity* ConstantVel)
{
    ...
    {for (int32x Idx = 0; Idx < 3; ++Idx)
    {
        ...
        SliderRect.y = CurrY;
        if (SliderOn(-1, 1, Axis[Idx], SliderRect, IDGen))
        {
            PushUndoPos("Change axis", eSliderMotion, ConstantVel->GetUID());
            edit::MarkModified();
...
blendnode_edit_visitor::VisitNode(aimat_ik* AimAt)
{
    ...
    {for (int Idx = 0; Idx < 3; ++Idx)
    {
        ...
        if (SliderOn(-CubeDimension, CubeDimension, AimPosition[Idx],
                     FillRectToRight(ControlX, drawArea.w, CurrY, 26), IDGen))
        {
            PushUndoPos("aimat option", eSliderMotion, AimAt->GetUID());
            edit::MarkModified();
...
blendnode_edit_visitor::VisitNode(event_renamer* Renamer)
{
    ...
    {for (int Idx = 0; Idx < Renamer->NumRenames(); ++Idx)
    {
        ...
        if (EditLabelOn(Str, FillRectToRight(ControlX, drawArea.w - 50, CurrY, 24), IDGen))
        {
