...
static void
UpdateWindowTitle(HWND hWnd,
                  int UpdateMajor,
                  int UpdateMinor,
                  int UpdateBuildNumber,
                  int UpdateCustomization)
{
    char newVer[1024];
    sprintf(newVer, "Granny Animation Studio (version %d.%d.%d.%d available!)",
            UpdateMajor, UpdateMinor, UpdateBuildNumber, UpdateCustomization);
    SetWindowText(hWnd, newVer);
}

static DWORD WINAPI
UpdateCheckThread(LPVOID lpParameter)
{
    HWND window = (HWND)lpParameter;

    uint32 UpdateAvail = 0;
    GetApplicationInt("UpdateAvailable", 0, &UpdateAvail);
    if (UpdateAvail)
    {
        // There is already an update, probably no need to hit the website to check
        uint32 UpdateMajor;
        uint32 UpdateMinor;
        uint32 UpdateBuildNumber;
        uint32 UpdateCustomization;
        if (GetApplicationInt("UpdateMajor",         0, &UpdateMajor) &&
            GetApplicationInt("UpdateMinor",         0, &UpdateMinor) &&
            GetApplicationInt("UpdateBuildNumber",   0, &UpdateBuildNumber) &&
            GetApplicationInt("UpdateCustomization", 0, &UpdateCustomization))
        {
            // The update spec stored in the registry is valid
            if (UpdateMajor > ProductMajorVersion ||
                UpdateMinor > ProductMinorVersion ||
                UpdateBuildNumber > ProductBuildNumber ||
                UpdateCustomization > ProductCustomization)
            {
                // The update is still more recent than this version.
                UpdateWindowTitle(window, UpdateMajor, UpdateMinor, UpdateBuildNumber, UpdateCustomization);
                return 0;
            }
            else
            {
                // It is likely that we've updated to the new version!  Clear the update
                // flag, and fall through to the checks below...
                UpdateAvail = 0;
                SetApplicationInt("UpdateAvailable", 0);
            }
        }
        else
        {
            // The update specification stored in the registry is invalid.  Clear the
            // available flag so that we recheck when appropriate.  (Note that we could
            // technically collapse this with the case above, but the reason for falling
            // through is distinct, so I'm leaving it separate.)
            UpdateAvail = 0;
            SetApplicationInt("UpdateAvailable", 0);
        }
    }
    ...
    // See if enough time has elapsed between our last check and the current time.  We'll
    // use 1 week as our time span.
    FILETIME systemTime;
    {
        uint32 LastUpdateCheckHi;
        uint32 LastUpdateCheckLo;
        GetApplicationInt("LastUpdateCheckHi", 0, &LastUpdateCheckHi);
        GetApplicationInt("LastUpdateCheckLo", 0, &LastUpdateCheckLo);

        GetSystemTimeAsFileTime(&systemTime);

        uint64 LastUpdateCheck = (uint64(LastUpdateCheckHi) << 32) | LastUpdateCheckLo;
        uint64 systemTimeU64   = (uint64(systemTime.dwHighDateTime) << 32) | systemTime.dwLowDateTime;

        if (systemTimeU64 < LastUpdateCheck)
        {
            // odd, but just do the check, fall through below...
        }
        else
        {
            // One week check interval...
            uint64 CheckInterval = (7 * 24 * 60 * 60) * uint64(10000000);  // filetime ticks are 100 nanosecs
            uint64 Delta = systemTimeU64 - LastUpdateCheck;
            ...
            {
                // Time to check, fall through below...
            }
        }
    }

    SetApplicationInt("LastUpdateCheckHi", systemTime.dwHighDateTime);
    SetApplicationInt("LastUpdateCheckLo", systemTime.dwLowDateTime);

    HINTERNET hInternet = InternetOpen("Granny Animation Studio", INTERNET_OPEN_TYPE_DIRECT, 0, 0, 0);
    if (hInternet)
    {
        HINTERNET hURL = InternetOpenUrl(hInternet,
                                         "http://www.radgametools.com/granny/serverversion.js",
                                         0, 0, 0, 0);
        if (hURL)
        {
            char Buffer[4096];
...
#define Check(x)                                                                    \
                int x = -1;                                                         \
                {                                                                   \
                    char const* x ## str = strstr(Buffer, "var Latest" #x " = ");   \
                    if (x ## str)  {                                                \
                        x ## str += strlen("var Latest" #x " = ");                  \
                        x = atoi(x ## str);                                         \
                    }                                                               \
                ...
                if (MajorVersion >= 0 && MinorVersion >= 0 && Customization >= 0 && BuildNumber >= 0)
                {
                    // Do the check
                    ...
                    {
                        SetApplicationInt("UpdateAvailable", 1);
                        SetApplicationInt("UpdateMajor", MajorVersion);
                        SetApplicationInt("UpdateMinor", MinorVersion);
                        SetApplicationInt("UpdateBuildNumber", BuildNumber);
                        SetApplicationInt("UpdateCustomization", Customization);

                        UpdateWindowTitle(window, MajorVersion, MinorVersion, BuildNumber, Customization);
                    }
                }
            }
...
{
    hThread = CreateThread(0, 0, UpdateCheckThread, (LPVOID)window, 0, 0);
}
...
UIStartup(HWND hwnd)
{
    ...
    // TODO: Start drag accept
    // ActivateFileDrag(DragAccept);

    // Turn this on once we're sure it's solid
...
static int
App_GetPersitentInt(lua_State* L)
{
...
static int
App_SetPersitentInt(lua_State* L)
{
    char const* ValName = lua_tostring(L, -2);
...
{
    lua_register(State, "App_GetPersitentInt",  App_GetPersitentInt);
    lua_register(State, "App_SetPersitentInt",  App_SetPersitentInt);
