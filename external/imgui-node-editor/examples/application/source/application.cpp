    ...
    , m_Renderer(CreateRenderer())
{
	...
	for(int i = 0; i<argc; ++i){
		m_LaunchArguments.push_back({argv[i]});
	}
}
...
bool Application::Create(int width /*= -1*/, int height /*= -1*/)
{
	...
	auto handle = m_Platform->GetMainWindowHandle();

	m_Width = 0;
	m_Height = 0;
	
	glfwGetWindowSize((GLFWwindow*)handle, &m_Width, &m_Height);
...
void Application::Frame()
{
	...
	// Call OnFrame with calculated delta time
	OnFrame(deltaTime);
