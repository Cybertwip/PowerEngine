...
void SelfContainedSkinnedMeshBatch::append(std::reference_wrapper<SkinnedMesh> meshRef) {
	...
	mBatchBoneIds.insert(mBatchBoneIds.end(),
						 mesh.get_flattened_bone_ids().begin(),
						 mesh.get_flattened_bone_ids().end());
	mBatchBoneWeights.insert(mBatchBoneWeights.end(),
...
void SelfContainedSkinnedMeshBatch::remove(std::reference_wrapper<SkinnedMesh> meshRef) {
	...
	auto it = std::find_if(mMeshes.begin(), mMeshes.end(),
						   [&mesh](const std::reference_wrapper<SkinnedMesh>& m) {
		return &m.get() == &mesh;
	});
	...
	for (auto& index : mBatchIndices) {
		if (index >= vertexStartIdx + numVertices) {
			...
			index = 0; // Or handle appropriately if needed
		}
	}
	...
	mBatchBoneIds.erase(
						mBatchBoneIds.begin() + vertexStartIdx * 4,
						mBatchBoneIds.begin() + (vertexStartIdx + numVertices) * 4
						);
	mBatchBoneWeights.erase(
							mBatchBoneWeights.begin() + vertexStartIdx * 4,
							mBatchBoneWeights.begin() + (vertexStartIdx + numVertices) * 4
							);
...
void SelfContainedSkinnedMeshBatch::upload_vertex_data() {
	...
	mShader.persist_buffer("aColor", nanogui::VariableType::Float32, { mBatchColors.size() / 4, 4 },
						   mBatchColors.data());
	mShader.persist_buffer("aBoneIds", nanogui::VariableType::Int32, { mBatchBoneIds.size() / 4, 4 },
						   mBatchBoneIds.data());
	mShader.persist_buffer("aWeights", nanogui::VariableType::Float32, { mBatchBoneWeights.size() / 4, 4 },
						   mBatchBoneWeights.data());
...
void SelfContainedSkinnedMeshBatch::draw_content(const nanogui::Matrix4f& view,
	...
	for (size_t i = 0; i < mMeshes.size(); ++i) {
		...
		upload_material_data(mShader, mesh.get_mesh_data().get_material_properties());
