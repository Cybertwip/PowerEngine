...
mApplicationClickRegistrator(applicationClickRegistrator) {
	...
	mResourcesPanel = std::make_shared<ResourcesPanel>(parent.parent()->get(), screen, *rootNode, mActorVisualManager, mSceneTimeBar, animationTimeProvider, mMeshActorLoader, mShaderManager, deepMotionApiClient, powerAi, mUiManager);
...
void StatusBarPanel::toggle_resources_panel(bool active) {
	...
	if (active) {
		mAnimationFuture = std::async(std::launch::async, [this]() {
			auto target = nanogui::Vector2i(0, parent()->get().parent()->get().fixed_height() * 0.45f - fixed_height());
