...
: Panel(parentWidget, "Camera"), mActiveActor(std::nullopt), mIsControlling(false) {
	...
	// -- Checkboxes --
	mCheckboxContainer = std::make_shared<nanogui::Widget>(std::make_optional<std::reference_wrapper<nanogui::Widget>>(*this));
	mCheckboxContainer->set_layout(std::make_unique<nanogui::BoxLayout>(nanogui::Orientation::Horizontal, nanogui::Alignment::Middle, 0, 6));
...
void CameraPanel::update_values_from(const CameraComponent& camera) {
	...
	mOrthoCheck->set_checked(camera.is_orthographic());
	mDefaultCheck->set_checked(camera.is_default());
	
	// Disable FOV controls if the camera is orthographic
	mFovBox->set_enabled(!camera.is_orthographic());
	mFovLabel->set_enabled(!camera.is_orthographic());
}
