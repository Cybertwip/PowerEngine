...
HierarchyPanel::HierarchyPanel(nanogui::Widget &parent) : Panel(parent, "Hierarchy") {
    ...
    mTreeView->set_layout(new nanogui::GridLayout(
                nanogui::Orientation::Horizontal, 1 /*columns */,
                nanogui::Alignment::Fill, 3, 3));
}

bool HierarchyPanel::mouse_drag_event(const nanogui::Vector2i &p, const nanogui::Vector2i &rel, int button,
                             int modifiers) {
    // Disable dragging
...
void HierarchyPanel::populate_tree(Actor *actor, nanogui::TreeViewItem *parent_node) {
    ...
    nanogui::TreeViewItem *node = parent_node ? parent_node->add_node("actor->name()") : mTreeView->add_node("actor->name()");
