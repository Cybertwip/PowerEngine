...
public:
    /**
     * @brief Constructs a NodeProcessor and registers all available node types.
     */
    NodeProcessor();
...
private: 
    // A function pointer that can create a CoreNode.
    using CreatorFunc = std::function<std::unique_ptr<CoreNode>(UUID)>;

    /**
     * @brief Registers a reflectable C++ type, making it available to the blueprint system.
     * This creates the necessary runtime functions to instantiate the object and
     * dispatch calls to its methods and properties.
     * @tparam T The C++ class to register (e.g., Transform).
     */
    template<typename T>
    void register_blueprint_type();
