...
// The constructor now correctly takes void* for the device, matching the header.
GizmoManager::GizmoManager(nanogui::Widget& parent, void* device, unsigned long colorPixelFormat, ActorManager& actorManager)
: mDevice(device), mActorManager(actorManager) {
	
	// Cast the opaque device pointer to its actual Metal type for use in this file.
	id<MTLDevice> mtlDevice = (id<MTLDevice>)mDevice;
	...
	id<MTLLibrary> library = [mtlDevice newDefaultLibraryWithBundle:[NSBundle mainBundle] error:&error];
	if (!library) {
	...
	mPipelineState = [mtlDevice newRenderPipelineStateWithDescriptor:pipelineDescriptor error:&error];
	if (!mPipelineState) {
	...
	mDepthDisabledState = [mtlDevice newDepthStencilStateWithDescriptor:depthDesc];
...
void GizmoManager::create_translation_gizmo() {
	...
	for (int i = 0; i < 3; ++i) { // 0=X, 1=Y, 2=Z
		...
		// The triangle-based cone for the arrowhead
		...
		id<MTLBuffer> buffer = [mtlDevice newBufferWithBytes:vertices.data()
...
void GizmoManager::create_rotation_gizmo() {
	...
	for (int i = 0; i < 3; ++i) { // 0=X, 1=Y, 2=Z
		...
		id<MTLBuffer> buffer = [mtlDevice newBufferWithBytes:vertices.data() length:vertices.size() * sizeof(glm::vec3) options:MTLResourceStorageModeShared];
...
void GizmoManager::draw(void* encoder, const glm::mat4& view, const glm::mat4& projection) {
	if (!mActiveActor.has_value() || mCurrentMode == GizmoMode::None) {
		return;
	}
	
	// Cast the opaque encoder pointer to its actual Metal type
	id<MTLRenderCommandEncoder> mtlEncoder = (id<MTLRenderCommandEncoder>)encoder;
	...
	[mtlEncoder pushDebugGroup:@"DrawGizmo"];
	[mtlEncoder setRenderPipelineState:(id<MTLRenderPipelineState>)mPipelineState];
	[mtlEncoder setDepthStencilState:(id<MTLDepthStencilState>)mDepthDisabledState];
	...
	[mtlEncoder setVertexBytes:&uniforms length:sizeof(uniforms) atIndex:1];
	...
	for (int i = 0; i < 3; ++i) {
		...
		simd_float4 color = { (float)(i == 0), (float)(i == 1), (float)(i == 2), 1.0f }; // R, G, B for X, Y, Z
		[mtlEncoder setFragmentBytes:&color length:sizeof(color) atIndex:0];
		
		// Set the vertex buffer for this axis
		[mtlEncoder setVertexBuffer:(id<MTLBuffer>)active_meshes[i].vbo offset:0 atIndex:0];
		
		// Issue the draw call
		[mtlEncoder drawPrimitives:(MTLPrimitiveType)active_meshes[i].primitiveType
					   vertexStart:0
