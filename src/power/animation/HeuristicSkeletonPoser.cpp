...
void HeuristicSkeletonPoser::apply() {
	...
	for (auto& cb : classifiedBones) {
		if (cb.type == BoneType::Root || (cb.type == BoneType::RootChain && cb.bone != rootBone)) {
			mSkeleton.trim_bone(*cb.bone);
		}
	}
...
void HeuristicSkeletonPoser::ApplyTPose(const std::vector<ClassifiedBone>& classifiedBones, ISkeleton& skeleton) {
	for (const auto& cb : classifiedBones) {
		...
		switch (cb.type) {
			case BoneType::Root:
				desiredRotation = glm::quat(glm::radians(glm::vec3(90, 0, -90)));
				break;
