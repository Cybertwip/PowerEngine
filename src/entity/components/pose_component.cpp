...
void PoseComponent::set_animation_component(std::shared_ptr<AnimationComponent> animation_component)
{
...
void PoseComponent::set_armature_root(std::shared_ptr<Entity> armature_root)
{
	armature_root_ = armature_root;
}
void PoseComponent::update(std::shared_ptr<Entity> entity)
{
...
std::shared_ptr<Entity> PoseComponent::get_root_entity()
{
...
std::shared_ptr<AnimationComponent> PoseComponent::get_animation_component() {
	return animation_component_;
}

void PoseComponent::add_and_replace_bone(const std::string &name, const glm::mat4 &transform)
{
	auto animation = (animation_component_) ? animation_component_->get_animation() : nullptr;
	if (animation)
	{
		animation->add_and_replace_bone(name, transform, animator_->get_current_frame_time());
	}
}
void PoseComponent::sub_current_bone(const std::string &name)
{
	...
	if (animation)
	{
		animation->sub_bone(name, animator_->get_current_frame_time());
	}
}

std::shared_ptr<Entity> PoseComponent::find(int bone_id)
{
	return find(bone_id, armature_root_);
}
std::shared_ptr<Entity> PoseComponent::find(int bone_id, std::shared_ptr<Entity> entity)
{
