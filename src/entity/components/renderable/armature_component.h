...
namespace anim
{
    ...
    public:
        ...
        void set_pose(std::shared_ptr<PoseComponent> pose);
		...
		std::shared_ptr<PoseComponent> get_pose();
        ...
        void add_and_replace_bone(std::shared_ptr<Entity> entity);
		
		
		std::shared_ptr<Component> clone() override {
			auto clone = std::make_shared<ArmatureComponent>();
			
			clone->name_ = name_;
			clone->scale_ = scale_;
			clone->rotation_ = rotation_;
			clone->offset_ = offset_;
			clone->bind_pose_ = bind_pose_;
		}
		

    private:
		std::shared_ptr<PoseComponent> pose_;
        int id_{-1};
        std::string name_;
