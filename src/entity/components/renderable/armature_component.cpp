...
namespace anim
{
    ...
    void ArmatureComponent::set_pose(std::shared_ptr<PoseComponent> pose)
    {
        pose_ = pose;
    }
    ...
    std::shared_ptr<PoseComponent> ArmatureComponent::get_pose()
    {
        return pose_;
    }
    ...
    void ArmatureComponent::update(std::shared_ptr<Entity> entity)
    {
        ...
        for (int i = 0; i < rotation_.size(); i++)
        {
            ...
            if (entity->is_selected_ || entity->get_mutable_root()->is_selected_ || pose_->get_root_entity()->get_mutable_parent()->is_selected_)
            {
    ...
    void ArmatureComponent::add_and_replace_bone(std::shared_ptr<Entity> entity)
    {
        pose_->add_and_replace_bone(name_, entity->get_local());
    }
}
