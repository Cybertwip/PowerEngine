...
namespace anim
{
    ...
    struct FramebufferSpec
    {
        ...
        FramebufferSpec(uint32_t framebuffer_width, uint32_t framebuffer_height, uint32_t msaa_samples, std::initializer_list<FramebufferTextureSpec> color_attachments, FramebufferTextureSpec depth_attachment = FramebufferTextureFormat::None)
            : width(framebuffer_width), height(framebuffer_height), color_attachments_spec(color_attachments), depth_attachment_spec(depth_attachment), samples(msaa_samples) {}
    ...
    public:
        ...
        const int read_pixel(unsigned int x, unsigned int y, uint32_t color_attachment_idx)
        {
            ...
            glReadBuffer(GL_COLOR_ATTACHMENT0 + color_attachment_idx);

			int data[16] = {0};
			glReadPixels(x - 4, get_height() - y - 8, 4, 4, GL_RED_INTEGER, GL_INT, data);
    ...
    private:
		...
		void setupIntermediateFramebuffer(int width, int height);
        void set_quad_VAO();
