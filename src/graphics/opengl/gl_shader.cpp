...
namespace {
std::string InjectHighPrecision(const std::string& shaderCode, const std::string& shaderType) {
	std::size_t versionPos = shaderCode.find("#version");
	std::size_t lineEndPos = shaderCode.find("\n", versionPos);
	
	std::string modifiedShader;
	
	if (versionPos != std::string::npos && lineEndPos != std::string::npos) {
		// Extract version line
		std::string versionLine = shaderCode.substr(versionPos, lineEndPos - versionPos + 1);
		
		// Rebuild shader with precision inserted after version directive
		modifiedShader = versionLine; // Include the version line
		
		// For fragment shaders, inject highp qualifiers; vertex shaders usually default to highp
		if(shaderType == "fragment") {
			modifiedShader += "\nprecision highp float;\n";
			modifiedShader += "precision highp int;\n";
		}
		
		// Append the rest of the shader code
		modifiedShader += shaderCode.substr(lineEndPos + 1);
	} else {
		// If no version directive found, return original code (though this should be an error)
		modifiedShader = shaderCode;
	}
...
void GLShader::set_float_array(const std::string &name, const float *values, unsigned int count) const
{
	glUniform1fv(glGetUniformLocation(id_, name.c_str()), count, values);
}
