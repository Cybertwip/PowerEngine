...
namespace{
bool checkAnisotropicFilteringSupport() {
	GLint numExtensions;
	glGetIntegerv(GL_NUM_EXTENSIONS, &numExtensions);
	
	for (GLint i = 0; i < numExtensions; i++) {
		const char* extensionName = reinterpret_cast<const char*>(glGetStringi(GL_EXTENSIONS, i));
		if (strcmp(extensionName, "GL_EXT_texture_filter_anisotropic") == 0) {
			return true;
		}
	}
	return false;
...
inline constexpr void SetTexParameteri(const FramebufferTextureFormat &format, GLfloat maxAnisotropy = 1.0f) // Default to 1.0f for non-anisotropic
{
	switch (format)
	{
		case FramebufferTextureFormat::RED_INTEGER:
		...
		default:
			...
			if (maxAnisotropy > 1.0f) { // Apply anisotropic filtering if supported and beneficial
...
void AttachColorAttachment(uint32_t &id, int samples, uint32_t width, uint32_t height, const FramebufferTextureSpec &spec, int index, GLfloat maxAnisotropy = 1.0f)
{
	...
	{
		SetTexParameteri(spec.texture_format, maxAnisotropy);
...
void AttachDepthRBO(uint32_t &id, int samples, uint32_t width, uint32_t height, const FramebufferTextureSpec &spec)
{
	bool is_msaa = samples > 1;
	GLenum format = ConvertFormat(spec.texture_format);
	GLenum internalFormat = ConvertInternalFormat(spec.texture_format);
...
bool GenFramebuffer(uint32_t &id, const FramebufferSpec &spec, std::vector<uint32_t> &color_id, uint32_t &depth_id)
{
	...
	{
		AttachColorAttachment(color_id[i], spec.samples, spec.width, spec.height, spec.color_attachments_spec[i], i, maxAnisotropy);
...
{
	if (spec_.samples <= 1) return; // Only necessary for MSAA
	
	is_error_ = util::GenFramebuffer(renderer_id_, spec_, color_id_, depth_id_);
	screen_texture_id_ = color_id_[0];

	// Generate the intermediate framebuffer for resolving multisamples
	glGenFramebuffers(1, &intermediate_renderer_id_);
	glBindFramebuffer(GL_FRAMEBUFFER, intermediate_renderer_id_);
	
	// Generate and bind the texture to store the resolved render
	glGenTextures(1, &intermediate_color_id_);
	glBindTexture(GL_TEXTURE_2D, intermediate_color_id_);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, spec_.width, spec_.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, intermediate_color_id_, 0);
	
	// Set texture parameters (optional, depending on your needs)
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
